...
$$
denote a Jacobi matrix evaluated at $\vi x^*$. Then $\vi x^*$ is *stable*, if all eigenvalues $\lmbd_i$, where $i \in \oneToN{n}$^[For conciseness, we use the following notation $\oneToN{n} := \set{1, \dots, n}.$], of the matrix $\Jacobi^*$ satisfy $\reOf{\lmbd_i} < 0$.
:::
...
Consider a dynamical system ([-@eq-autonomous-difference-sys]) with a fixed point $\vi x^*$. Let $\Jacobi^* = \jacobi \vi f(\vi x^*)$ denote the Jacobi matrix evaluated at $\vi x^*$. Then $\vi x^*$ is stable, if all eigenvalues^[Eigenvalues of fixed points of discrete-time dynamical systems are often called *multipliers*, see @Sevcik2021.] $\lambda_i$, where $i \in \oneToN{n}$, of the matrix $\Jacobi^*$ satisfy $\absval{\lambda_i} < 1$.
:::
...
A fixed point $\vi x^*$ of the system ([-@eq-autonomous-difference-sys]) is called **hyperbolic**, if none of the eigenvalues corresponding to the Jacobi matrix $\Jacobi^* = \jacobi \vi f(\vi x^*)$ has unit magnitude.
:::
...
where $\vi x_m = (x_{m, 1}, x_{m, 2})\Tr$ and $\vi f : \R^2 \to \R^2$ is smooth. Moreover, let us assume there exists a *hyperbolic equilibrium* $\vi x^* = \vi f(\vi x^*)$ of the system ([-@eq-2d-discrete-dyn-sys]) and let $\Jacobi^* = \jacobi \vi f(\vi x^*)$ denote the corresponding Jacobi matrix evaluated at $\vi x^*$. Then $\Jacobi^*$ has two eigenvalues $\lambda_1, \lambda_2$, which satisfy
$$
\det \brackets{\Jacobi^* - \lambda \ident_2} = \lambda^2 - \trace \Jacobi^* \lambda + \det \Jacobi^*.
$$
Here, $I_2$ corresponds to a 2-by-2 identity matrix, $\trace \Jacobi^* = \lambda_1 + \lambda_2$  is the trace of the Jacobian and finally, $\det \Jacobi^* = \lambda_1 \lambda_2$ denotes the determinant of $\Jacobi^*$.
...
hidedecorations!(ax)
scatterlines!(ax, solve([0.5 0; 0 0.25], x0, n), colormap=:Zissou1, color=1:n)

ax = Axis(fig[1,2], title="spiral sink")
hidedecorations!(ax)
scatterlines!(ax, solve([0.5 0.25; -0.25 0.5], x0, n), colormap=:Zissou1, color=1:n)

ax = Axis(fig[1,3], title="saddle")
hidedecorations!(ax)
scatterlines!(ax, solve([1.25 0; 0 0.25], x0, n), colormap=:Zissou1, color=1:n)
scatterlines!(ax, solve([1.25 0; 0 0.25], x0 .* [-1.0, 1.0], n), colormap=:Zissou1, color=1:n)

ax = Axis(fig[1,4], title="source")
hidedecorations!(ax)
scatterlines!(ax, solve([1.5 0; 0 1.25], x_eq, n), colormap=:Zissou1, color=1:n)

ax = Axis(fig[1,5], title="spiral source")
hidedecorations!(ax)
scatterlines!(ax, solve([1.5 0.75; -0.75 1.5], x_eq, n), colormap=:Zissou1, color=1:n)
...
![Poincare map for cycle???](diagrams/poincare_maps.drawio.svg){#fig-poincare-map .final}
