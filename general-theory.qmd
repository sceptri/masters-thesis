...
$$
which assumes, among other facts, a constant capacity of the environment $K$. Should this value be subject to change in time $K = K(t)$ (for example by external influences), yielding
$$
\dot{n}(t) = a \cdot \brackets{1 - \frac{n(t - T)} {K(t)}} n(t),
$$
...
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; set $s \gets s + \tau_j$ and $\vi \xi(\theta) = \begin{cases}
...
Last, but not least, we shall discuss the Newton's, or Newton-Raphson, method for finding roots of a given equation (or a system thereof). Suppose we are given a system of equations
$$
\begin{aligned}
f_1(x_1, \dots, x_n) &= 0 \\
f_2(x_1, \dots, x_n) &= 0 \\
