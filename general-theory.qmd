...
Solving, or numerically integrating, ordinary differential equations can be seen as a backbone of this thesis. As such, we will introduce two approaches, Euler method and Runge-Kutta method. This subsection is mostly based on @Butcher2016 and @Trefethen1994.
...
Repeating this process on the entire interval $[t_0, t_N]$ gives us @alg-euler-method. Let us note that often an equidistant stepsize is used, i.e. $\diff t_k = \diff t = \frac {t_N - t_0} N$.

::: {.callout-important appearance="simple" #alg-euler-method}
##### Euler method (explicit)
**Input:** ODE in form ([-@eq-ode-problem-to-solve]), interval $[t_0, t_N]$, stepsizes $\seqnc{\diff t}{i}{1}{N}$ \
...
<!-- TODO: Check indices of \diff t_k -->
:::{#def-ode-solver-error}
Let us consider an ODE problem of form ([-@eq-ode-problem-to-solve]) and the solution $\vi x(t)$ to the initial-value problem. Assume $\vi x_k$ is an approximate solution at time $t_k$ and that $\vi x_{k+1}$ is given by $\vi x_{k+1} = \vi x_k + \diff t_{k+1} \cdot \incrementF{t_k, \vi x (t_k), \diff t_{k+1}, \vi F}$, where $\incrementFunc$ is the *increment* function^[In general, the increment function $\incrementFunc$ at the $k+1$-st step may depend on $t_0, \dots, t_k$, $\vi x_0, \dots, \vi x_k$, and $\diff t_1, \dots, \diff t_{k + 1}$.], which propagates the solution from $t_k$ to the next $t_{k+1}$ and represents the chosen method. Then *local truncation error* (LTE) is defined as
$$
\locTrErr_{k} = \vi x(t_{k+1}) - \vi x(t_k) - \diff t_{k+1} \cdot \incrementF{t_k, \vi x (t_k), \diff t_{k+1}, \vi F}
$$ {#eq-loc-tr-err}
and represents the error the increment function causes when performing a single step. *Global error* is then defined as the cummulative LTE from the initial condition, and can be written as follows:
$$
