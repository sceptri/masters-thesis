...
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; define ODE
$$
\dvi y(t) = \vi f(\vi y(t), \vi \xi(t - \tau_1), \dots, \vi \xi(t - \tau_m))
$$ {#eq-method-of-steps-intermediate}
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; with initial value $\vi y(s) = \vi \xi(s)$; \
...
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; set $s \letDef s + \tau_j$ and
$$
\vi \xi(\theta) = \begin{cases}
...
What is the form of the random process $\rZ(t)$, such that $\diff \rZ(t) = \rve(t) \, \diff t$, where $\rve(t)$ is a **white Gaussian noise**? To a careful reader, this may seem at least a little reminiscent of a Wiener process. In other words, differences $\diff Z(t)$ are *Gaussian white noise*^[A random variable is called Gaussian white noise if it has normal distribution with zero mean.] and if we require (almost sure) continuity and $\rZ(0) = 0$, we get that $\rZ(t)$ is the Wiener process -- that is $\diff \rW(t)$ have the same properties as $\rve(t) \diff t$. With the limit transition $\lim_{\diff t \to 0}$ in the $\ltwo$ sense we get, arguably after much more mathematics,
$$
\d \rW(t) = \rve(t) \dd t,
$$
which can re-written in form of a *distributional derivative*. Now let us return to the original stochastic differential equation, that now becomes
$$
\d \rX(t,\omega) = r  \rX(t, \omega) \dd t + \sigma  \rX(t, \omega) \overbrace{\rve(t, \omega) \dd t}^{\d \rW(t)},
$$
which we are tasked with solving for a random process $\rX(t)$, $t \geq 0$, that satisfies this SDE. Moreover
