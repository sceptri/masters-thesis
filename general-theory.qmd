...
The system of ODEs ([-@eq-autonomous-ode-system]) induces an *invertible continuous-time dynamical system*.
:::
...
Analogously to the dynamical system case, the evolution operator $\evolOp$ of an autonomous semidynamical system is called a *semiflow*.
...
which represents the assumption the all points, where $\vi F$ is evaluated, are first order approximations to the solution. It can be shown that they significantly simplify the derivation of order conditions ([-@eq-runge-kutta-order-condition-4-stage]), but for low order, they are not necessary.
...
\end{aligned} \right\} \; \vi x_1 = \vi x_0 + \diff t \brackets{\frac 1 6 \vi k_1 + \frac 2 6 \vi k_2 + \frac 2 6 \vi k_3 + \frac 1 6 \vi k_4}.
...
### Solving DDEs with Method of Steps
...
::: {.callout-important appearance="simple" #alg-method-of-steps}
##### Method of steps
**Input:** DDE in form ([-@eq-dde-problem-autonomous]), ODE numerical integration method $\mathrm{odeSolver}$\
**Output:** Sequence of $N \letDef k \sum_{i = 1}^m N_i = k \cdot N_{\tau}$ points $\seqnc{\vi x}{i}{1}{N}$ \
**begin** \
&emsp;&emsp;set $\vi \xi \letDef \vi \phi$ and $s \letDef \sigma$; \
&emsp;&emsp;define $A_{x,y} = (x - 1) N_{\tau} + \sum_{b = 1}^{y} N_b$ \
&emsp;&emsp;**for** $i = 1$ **to** $k$ **do** \
&emsp;&emsp;&emsp;&emsp;**for** $j = 1$ **to** $m$ **do** \
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; define ODE IVP by $\vi y(s) = \vi \xi(s)$ and
$$
\dvi y(t) = \vi f(\vi y(t), \vi \xi(t - \tau_1), \dots, \vi \xi(t - \tau_m));
$$ {#eq-method-of-steps-intermediate}
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; apply $\mathrm{odeSolver}$ to ([-@eq-method-of-steps-intermediate]) yielding $N_j$ points $\seqnc {\vi x} a {A_{i, j - 1}} {A_{i,j}}$; \
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; set $s \letDef s + \tau_j$ and $\vi \xi(\theta) = \begin{cases}
...
$$
\d \rX(t,\omega) = r  \rX(t, \omega) \dd t + \sigma  \rX(t, \omega) \overbrace{\rve(t, \omega) \dd t}^{\d \rW(t)},
$$
...
Let us now turn our attention back to the noisy measurements problem. Using discussed concepts, this can be represented as a discrete-time approximation of an Itô process. In particular, we will use the **Euler-Maruyama** approximation. Consider an Itô process $\rX = \set{\rX(t) \divider t_0 \leq t \leq T}$ satisfying the scalar SDE
$$
...
$$
\rY_{n+1} = \rY_n + a(t_n, \rY_n) \diff t_n + b(t_n, \rY_n) \diff \rW_n,
$$ {#eq-em-iteration}
and as such $\rY$ can be interpreted as a Markov chain. This can be easily extended to vector-valued SDE,
$$
...
::: {.callout-important appearance="simple" #alg-euler-maruyama-method}
...
**begin** \
&emsp;&emsp;set $\vr Y_0 \letDef \vr X_0$ by ([-@eq-euler-maruyama-vec-problem]) \
&emsp;&emsp;**for** $i = 0$ **to** $N - 1$ **do** \
&emsp;&emsp;&emsp;&emsp;$\vr Y_{n+1} \letDef \vr Y_n + \vi a(t_n, \vr Y_n) \diff t_n + \vi B(t_n, \vr Y_n) \diff \vr W_n$ \
&emsp;&emsp;**end** \
**end**
:::
...
$$
denotes the length of the $k$-th bracket and $l_k \to 2 \delta$ for $k \to \infty$.

::: {.callout-important appearance="simple" #alg-bisection-method}
##### Bisection method
...
**begin** \
&emsp;&emsp;set $a_0 \letDef a, b_0 \letDef b$ \
&emsp;&emsp;**for** $i = i$ **to** $k$ **do** \
&emsp;&emsp;&emsp;&emsp;calculate $x_i^- \letDef \frac {a_{i - 1} + b_{i - 1}} 2 - \delta$ & $x_i^+ \letDef \frac {a_{i - 1} + b_{i - 1}} 2 + \delta$ \
...
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;set^[In the case of equality, both options for a new bracket are equally valid.] $[a_i, b_i] \letDef [x_i^-, b_{i - 1}]$ \
&emsp;&emsp;&emsp;&emsp;**end** \
