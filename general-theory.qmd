...
As the title may suggest, computational aspects of ODEs, DDEs and other mathematical problems will play a key role in this thesis. For completeness' sake, a brief overview of numerical integrators (also called *solvers*) for ODEs, DDEs and stochastic differential equations (SDEs) will be given, as well as basic methods of one-dimensional optimization.

### Numerical Integration

#### Solving ODEs

Solving, or numerically integrating, ordinary differential equations can be seen as a backbone of this thesis. As such, we will introduce two approaches, Euler method and Runge-Kutta method. This subsection is mostly based on @Butcher2016.
...
$$
Setting $t = t_0 + \diff t$ yields 
$$
\vi x(t_0 + \diff t) \approx \vi x(t_0) + \dvi x(t_0) (t_0 + \diff t  - t_0) = \vi x(t_0) + \vi F(t_0, \vi x(t_0)) \diff t.
$$

Repeating this process on the entire interval $[t_0, t_N]$ and using $\diff t = \frac {t_N - t_0} N$ gives^[In practice, the step size $\diff t$ maybe dependent (implictly or explicitly) on the iteration, i.e. $\diff t = \diff t_k$.] us @alg-euler-method.

::: {.callout-important appearance="simple" #alg-euler-method}
##### Euler method
**Input:** ODE in form ([-@eq-ode-problem-to-solve]), interval $[t_0, t_N]$, granularity $N$ (or possibly stepsizes $\seqnc{\diff t}{k}{1}{N}$) \
**Output:** Sequence of $N$ points $\seqnc{\vi x}{k}{1}{N}$ \
**begin** \
&emsp;&emsp;Ahoj \
&emsp;&emsp;&emsp;&emsp;$x^2$ \
&emsp;&emsp;and this \
**end**
:::
