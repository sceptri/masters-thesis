...
where $\nonzeroEl$ denotes a generally non-zero element. Thus $\jacobi \vi H$ is sparse, and because it corresponds to the linear operator ([-@eq-ode-continuation-operator]) it also has a one-dimensional null-space satisfying $\vi H(\vi X) = 0$ (at generic points). Let us note that by later also adding a constraint equation stemming from the chosen *continuation* method, see @sec-continuation, we will get a fully determined system. 
...
$$
\sum_{i = 0}^k \alpha_i \vi x_i = h \sum_{i = 0}^k \beta_i \cdot \brackets{\jacobi_{\tau_0} \vi f^* t_i + \sum_{j = 1}^H \jacobi_{\tau_i} \vi f^* \tilde{\vi x}(t_i - \tau_j)},
$$ {#eq-dde-lms}
where $\alpha_i, \beta_i \in \R$ are parameters and $\tilde{\vi x}(t_i - \tau_j)$ (when $(t_i - \tau_j) \notin \set{t_i}_{i = 1}^k$) are computed by polynomial interpolation with $s_- \in \N$ and $s_+ \in \N$ points to the left and right, respectively. To be precise, DDE-BIFTOOL uses so-called Nordsieck interpolation, see @Sieber2014.
...
TODO, see @Roose2007, @Fairgrieve1991, @Sieber2014, @Engelborghs2001, @Verheyden2005
Possibly also @Breda2006 and more
