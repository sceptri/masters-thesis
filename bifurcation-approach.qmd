...
$$ {#eq-dde-lms}
where $\alpha_i, \beta_i \in \R$ are parameters and $\tilde{\vi x}(t_i - \tau_j)$ (when $(t_i - \tau_j) \notin \set{t_i}$) are computed by polynomial interpolation with $s_- \in \N$ and $s_+ \in \N$ points to the left and right, respectively. To be precise, DDE-BIFTOOL uses so-called Nordsieck interpolation, see @Sieber2014.
...
<!-- TODO: Add ref to the previous section -->
As we have discussed in the previous section, see TODO, consider again the symmetrically coupled oscillators. This subsection is almost singularly based on @Zathurecky2025, which I am a co-author of.
