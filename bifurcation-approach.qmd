...
where $\nonzeroEl$ denotes a generally non-zero element. Thus $\jacobi \vi H$ is sparse, and because it corresponds to the linear operator ([-@eq-ode-continuation-operator]) it also has a one-dimensional null-space satisfying $\vi H(\vi X) = 0$ (at generic points). Let us note that by later also adding a constraint equation stemming from the chosen *continuation* method, see TODO, we will get a fully determined system. 
<!-- TODO: Add continuation ref -->
