...
with respect to (w.r.t.) parameter $\alpha \in \R$ as the solution to the following BVP ($\vi u_{\text{previous}}$ denotes the period-1 cycle for the previous value of $\alpha$):
$$
\rcases{
	\dvi u(s) - T \vi f(\vi u(s), \alpha) &= \vi 0, \; s \in [0,1], \\
	\vi u(0) - \vi u(1) &= \vi 0, \\
	\int_0^1 \scal{\vi u(\sigma)} {\dvi u_{\text{previous}}(\sigma)} \dd \sigma &= 0.
}
...
\bmtr{
	...
	\testInt{\dvi u_{\text{previous}}} & 0 & 0
},
