...
Once such method is available, any ODE numerical integrator which operates strictly on the mesh $\mcal T$ may be used --- these are typically called *linear multistep* (LMS) methods. For instance, @alg-dde-explicit-euler shows the explicit (forward) Euler method adapted for ([-@eq-dde-numerical-sol-problem]).
...
As can be seen from the previous subsection, a key quantity for a mathematical model of a single isolated neuron is likely to be the *membrane potential*, ideally effected by variables describing permeability of various ion channels. Indeed, conductance-based models, see @Skinner2006, are established on an equivalent circuit representation of a cell membrane, introduced by Hodgkin and Huxley @Hodgkin1952. Here, ion channels of an excitable cell are modeled as conductances and its lipid bilayer by a capacitor. In its simplest form, a conductance-based model represents a neuron by a single compartment with homogenous potential, disregarding ion movements between subcellular compartments, and captures only ion movements between the inside and outside of the cell.
...
The last model we shall consider is the *interneuron* model proposed by White et al., see @White1998. It was used to describe synchronization of oscillations and the loss thereof in a network of inhibitory neurons. Nonetheless, it allows us to fast-spiking neuron cells. The governing equations read as, see @Sevcik2025,
$$
\begin{aligned}
...
In practice, numerical experiments in neuroscience are often hard to set up due to the complicated nature of formulas of the systems and sheer number of their parameters. As such, throughout the work on this thesis, we developed a Julia package *NeuronToolbox.jl*, see its [Git repository](https://gitlab.com/sceptri-university/gamu/neuron-toolbox), with the aim to aid in this process. We illustrate this on a coupled pair of an interneuron model ([-@eq-interneuron-model]) and a Morris-Lecar model ([-@eq-morris-lecar]). The coupling is facilitated through a gap junction, see ([-@eq-coupling]), with the coupling strength $\lmbd = 0.01$ and corresponding delay $\tau = 0.01$.
...
The resulting network `coupled_pair` can then be used a normal function prescribing a (delay) differential equation with DifferentialEquations.jl. Also note that we provide various different models and network types, e.g., an $m\times m$ adjacency matrix coupling for a system of $m$ models.