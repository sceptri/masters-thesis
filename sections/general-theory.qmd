...
Suppose $s$ is a vanishing delay point of ([-@eq-dde-sol-continuity-problem]) with $\dvi x(t) = \vi f(t, \vi x(t), \vi x(\alpha(t)))$ and $\alpha$ is continuous. Then, there are infinitely many discontinuity points in any left neighborhood of $s$.
:::
...
$$
\alpha(s_{1,i}) = t_0 \quad \& \quad \dot{\alpha}(s_{1,i}) \neq 0,
...
\begin{aligned}
	...
	&+ \jacobi_{\vi \xi} \vi f(t, \vi x(t), \vi x(\alpha(t))) \dvi x(\alpha(t))^{\pm} \dot{\alpha}(t),
\end{aligned}
...
\begin{aligned}
	...
	&+ \jacobi_{\vi \xi} \vi f(s_{1,i}, \vi x(s_{1,i}), \vi x(t_0)) \dvi x(t_0)^{+} \dot{\alpha}(s_{1,i})
\end{aligned}
...
During the action potential, the corresponding part of the cell membrane opens its $\ce{Na^+}$-permissible ion channels. These, by combination of Coulomb's law and diffusion, enter the cell and positively charge it, which forces out cations of potassium $\ce{K^+}$ (drastically increasing the membrane potential). This, in turn, *hyper-polarizes* the cell membrane (the membrane potential is even lower than the resting value at this point). The entire process is then slowly reversed as sodium cations exit the cell and are replaced back by potassium cations. Note that the overpopulation of $\ce{Na^+}$ inside the cell restricts its ability to undergo the *action potential* until it sufficiently lowers, limiting the possible spiking frequency of an individual neuron.
...
As can be seen from the previous subsection, a key quantity for a mathematical model of a single isolated neuron is likely to be the *membrane potential*, ideally effected by variables describing permeability of various ion channels. Indeed, conductance-based models, see @Skinner2006, are established on an equivalent circuit representation of a cell membrane, introduced by @Hodgkin1952. Here, ion channels of an excitable cell are modeled as conductances and its lipid bilayer by a capacitor. In its simplest form, a conductance-based model represents a neuron by a single compartment with homogenous potential, disregarding ion movements between subcellular compartments, and captures only ion movements between the inside and outside of the cell.
...
The last model we shall consider is the *interneuron* model proposed by White et al., see @White1998. It was used to describe synchronization of oscillations and the loss thereof in a network of inhibitory neurons. Nonetheless, it allows us to fast-spiking neuron cells, which can be found, for instance, in the CA1 hippocampal subfield. The governing equations read as, see @Sevcik2025,
$$
\begin{aligned}
...
The type of coupling and its properties become crucial for the discussion on the communication between individual neurons. In particular, we are trying to model a situation in which the action potential of given neuron (called the *presynaptic* neuron) travels across its axon where it connects a dendrite of another neuron (the *postsynaptic* neuron). The point where one neuron connects to another is called a *synapse*.
