...
|$f^{(m)}(\cdot), \vi f^{(m)}(\cdot)$ | $m$-th derivative of a scalar-valued and vector-valued function, respectively |
...
|$\reOf{x}, \imOf{x}$ | real and imaginary part of complex variable $x$, respectively |
