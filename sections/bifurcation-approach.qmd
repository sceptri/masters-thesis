...
Notably, Záthurecký et al. [-@Zathurecky2025] have shown that there must locally exist a cycle manifold (with respect to coupling strength and an arbitrary parameter effecting the frequency of one oscillator) near zero coupling between the two nearly identical oscillators, such that their phases are synchronized. In particular, we shall focus primarily on the anti-phase synchronization, which may be used to explain the emergence of frequencies higher than physiological, see @Pribylova2024 and @Sevcik2025. This will be performed mainly by studying the phase shifts between the two oscillators.
