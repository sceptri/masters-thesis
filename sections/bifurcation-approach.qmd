...
The techniques of equilibrium localization for DDEs do not differ much from the ODE case. Indeed, assume we are trying to find a stable equilibrium. By time integration, see @sec-method-of-steps and @sec-integrating-ddes, we can get to a point $\vi x_t$ in a close neighborhood of the steady state solution. Then, we can apply the Newton-Raphson method, see @sec-newton-method, to the $n$-dimensional system
$$
...
A straightforward way to approximate the solution operator is to consider the matrix form of the numerical integration of the linearized equation. Such approach has been developed by Engelborghs et al., see @Engelborghs2002a, @Roose2007 and @Sieber2014, and relies on the linear multistep method, see @sec-integrating-ddes and @alg-dde-explicit-euler.
...
Currently, we have predicted a point $\tilde{\vi X}^i_{(0)}$ assumed to be close to the curve, which we need to *"correct"* such that it lies on $M$ within a certain specified degree of accuracy. This, in general, requires the use of a Newton-like method, see @sec-newton-method, as ([-@eq-continuation-problem]) might be highly nonlinear. However, a Newton-like methods can only be applied to fully determined systems, i.e., the number of equations must correspond to the number of unknown variables. Therefore, we add an extra scalar (remember $\vi H : \R^{n+1} \to \R^n$) condition
$$
g^i(\vi X) = 0
$$
...
One possibility is to select a hyper-plane $P^i$ passing through the point $\tilde{\vi X}^i_{(0)}$, such $\vi v^i$ is its normal vector. Then, we can find the corrected point $\vi X^{i+1}$ as the intersection of $P^i$ and the curve $M$. Further, any point $\vi X$ on $P^i$ must satisfy
$$
...
Overview of different prediction--correction methods for continuation (adapted from @Pribylova2021a, courtesy of Assoc. Prof. Přibylová).
:::
...
Notably, Záthurecký et al. [-@Zathurecky2025] have shown that there must locally exist a cycle manifold (with respect to coupling strength and an arbitrary parameter effecting the frequency of one oscillator) near zero coupling between the two oscillators, such that their phases are synchronized. In particular, we shall focus primarily on the anti-phase synchronization, which may be used to explain the emergence of frequencies higher than physiological, see @Pribylova2024 and @Sevcik2025. This will be performed mainly by studying the phase shifts between the two oscillators.

Moreover, this methodology enables us to study Arnold tongues corresponding to different shifts separately. This very fact was obscured by the simulation approach. Indeed if two manifolds of cycles overlap when projected onto the parameter space, the simulation approach will highlight only the one closer to the desired shift (if both are stable).
...
\end{aligned}
$$
describing $\lmbd$-weakly coupled oscillators $\vi x_1(t), \vi x_2(t)$. In particular, we shall again concern ourselves with the case of two weakly coupled interneuron models, see TODO. 

Let us begin with $\lmbd = 0$, i.e., decoupled oscillators. Then any shift may occur between them (as there is effectively no connection) and from the theoretical contributions of @Zathurecky2025, we know that an Arnold tongue with a stable synchronization structure must exists. Therefore, we choose a fixed discretization $\Lambda = \seqnc \lmbd i 0 {N_{\Lambda}}$ for $\lmbd$.

For each $\lmbd_i \in \Lambda$, we then let the system converge to a stable cycle from a fixed initial condition $\vi \phi$. After successful convergence, we then continue it with respect to $C_2$, which yields a slice of the unknown cycle manifold $M$.

::: {.callout-note #nte-lambda-discretization}
##### Discretization of $\lmbd$

Note that currently, we have "blindly" discretized $\lmbd$ and then for each element $\lmbd_i \in \Lambda$ of the discretization, we have attempted to converge onto a stable periodic orbit. Only afterwards could we compute the continuation with respect to $C_2$. Another approach would be first to continue the cycle with respect to $\lmbd$, producing a curve $C_{\lmbd}$ in the desired manifold $M$. Then, we could perform the continuation with respect to $C_2$ for each point of $C_{\lmbd}$, which is already discretized due to the nature of numerical continuation.
:::

Moreover, for each cycle on the $C_2$-continuation curve, we know its corresponding period $T$. This allows us to use shift searching methods, see @sec-shift-searching, with much better accuracy (compared to only estimated period). In particular, we partition the interval $[0,T]$ and for each of disjunct sub-intervals $Q_i \subseteq [0,T]$, we run the shift searching method. Finally, we select the shift with the least error.

<!-- TODO: Why C_1 in the code not C_2??? (and does it even matter for symmetric coupling?) -->
In addition, when there is no delay in the coupling $\vi K$ (i.e., the ODE case), one can also continue the LPC curves with respect to $\lambda$ and $C_2$. Note that these curves do not always originate from the zero coupling, as can be seen in Figures [-@fig-bifurcation-shift-ode] and [-@fig-bifurcation-shift-dde] (parts (C) and (D)). Unfortunately, DDE-BIFTOOL does not support continuation of LPC curves. Nevertheless, we may attempt to estimate these curves from the boundaries of stable and unstable limit cycles on the manifold $M$. 

Now, we can show the relation of $\lambda$ and $C_2$ to both period $T$ and shift $\beta$, see Figures [-@fig-bifurcation-shift-ode] and [-@fig-bifurcation-shift-dde] (parts (A) and (B)). Furthermore, we can project the cycle manifold onto the parameter plane $(C_2, \lmbd)$, where it draws out desired tongues. Let us remark tongues corresponding to different stable sub-manifolds of $M$ can overlap.

Figures [-@fig-bifurcation-shift-ode] and [-@fig-bifurcation-shift-dde] show this methodology applied to two weakly coupled interneuron modes, see TODO. The continuation was performed using MatCont, see @Dhooge2003 and @Dhooge2008, in the ODE case and DDE-BIFTOOL, see @Sieber2014, in the DDE case. In particular, we calculated the *optimal shift* in the first coordinate of both oscillators over $3T$-long trajectory. The region around zero-delay is numerically unstable for the continuation, see @sec-semidynamical-systems-localization. For implementation details, see the [Git repository](https://gitlab.ics.muni.cz/ndteam/gamu/shift-continuation) for @Zathurecky2025, potentially also Git repository associated with this thesis for specifics of plotting computed data.

The runtimes necessary to generate data for Figures [-@fig-bifurcation-shift-ode] and [-@fig-bifurcation-shift-dde] (parts (A) and (B)) are approximately 43 *seconds* for ODE case and 26 *minutes* for DDE, respectively. Note that although parts (C) and (D) of the same figures require a finer discretization $\Lambda$, in practice one would probably not need to use it. Indeed, there is no inherent increase in error when choosing a sparser discretization, whereas in the simulation-based approach it effected the entire result. As there is no dependence on results from other slices throughout the calculation, the parallelization is much easier and more efficient^[The implementation of the simulation-on-grid relies on using clever iteration order and multithreading locks, which results in delays when a thread is waiting to write to or read from the results matrices.]. The required runtime in MATLAB, see @MATLAB, in the DDE case is heavily dependent on the version of `dde23` solver, as discussed in @imp-dde23-improvements.

While there exists a Julia package DDEBifurcationKit.jl for continuation in DDEs, see @BifurcationKit, it currently does not support computation of Floquet multipliers for periodic orbits, see [this GitHub issue](https://github.com/bifurcationkit/DDEBifurcationKit.jl/issues/16) for more details. For completeness sake, we provide a reference single-threaded unoptimized implementation using this package as well, which can be found in the [Git repository](https://gitlab.ics.muni.cz/ndteam/gamu/shift-continuation) for @Zathurecky2025. The data generation process described above takes about 3 minutes for ODE case (on 4 threads).

```{julia}
#| echo: false
#| label: fig-bifurcation-shift-ode
#| fig-cap: Numerical continuation of the cycle manifold $M$ of the system TODO without delay for $C_1 = 1$ and free parameter $C_2$ using MatCont is shown in (A); (B) plots period $T$ with respect to $\lmbd$ and $C_2$. The colors of the continued section curves indicate stability (blue) or instability (red) of the limit cycles. The grey curves in panels (A) and (B) represent the projections of the black LPS curves onto $(C_2, \lmbd)$ delineating the Arnold tongues. The heatmaps of (C) the in-phase and (D) the anti-phase Arnold tongues illustrate the corresponding phase shift $\beta$. Adapted from @Zathurecky2025 (redrawn in Julia using data from the article).
#| width: 95%
...
#| fig-cap: Numerical continuation of the cycle manifold $M$ of the system TODO with delay $\tau = 0.025$ for $C_1 = 1$ and free parameter $C_2$ using DDE-BIFTOOL is shown in (A); (B) plots period $T$ with respect to $\lmbd$ and $C_2$. The colors of the continued section curves indicate stability (blue) or instability (red) of the limit cycles. The grey curves in panels (A) and (B) represent the projections of the black estimated LPS curves onto $(C_2, \lmbd)$ delineating the Arnold tongues. The heatmaps of (C) the in-phase and (D) the anti-phase Arnold tongues illustrate the corresponding phase shift $\beta$. Adapted from @Zathurecky2025 (redrawn in Julia using data from the article).
#| width: 95%
...
Comparison of heatmaps of phase shifts $\beta$ computed using the simulation-based approach, see @sec-simulation-approach and @fig-metacentrum-shift, and borders of anti-phase Arnold tongues obtained from the continuation of the cycle manifold $M$, see above.
:::