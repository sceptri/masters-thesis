...
All these concepts can now be combined. In @fig-indexed-shift, one can see the same heatmaps corresponding to the shift between two weakly coupled interneurons as in Figures [-@fig-cold-diff-shift] and [-@fig-cold-diff-shift-delay]. In particular, an indexed starter $\starter_{\indexer}$ was used with diagonal indexer of length $l_{\indexer} = 10$ on a 75-by-75 grid $\meshGrid$. An additive loss ([-@eq-starter-additive-loss]) was used for the starter in conjunction with a spiral iterator from the true center. The rest of the experiment setup follows @fig-cold-diff-shift.
