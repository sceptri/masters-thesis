...
We refer to the described method as the *diagonal indexer* of length $l_{\indexer} \in \N$ (example shown with $l_{\indexer} = 3$). Using a warm starter in conjunction with a diagonal indexer, yielding the minimization problem
$$
\starter_{\indexer \IndicesOf{\meshGrid}{E}}(\vi i) = \argmin_{\vi j \in \IndicesOf{\meshGrid}{E} \cap \indexer(\vi i)} \loss(\vi i, \vi j, \beta_{\vi j}),
$$ {#eq-warm-start-diagonal-indexer}
brings a very important computational benefit of *constant complexity*. Indeed, by employing a diagonal indexer, the warm starter^[The naive warm starter has a time complexity $O(N_{\Omega} \cdot N_{\Lambda})$.] requires comparison of at most $l_{\indexer}^2 - 1$ values of $\loss(\vi i, \vi j, \beta_{\vi j})$, where $l_{\indexer} \ll \minOf{N_{\Omega}, N_{\Lambda}}$.