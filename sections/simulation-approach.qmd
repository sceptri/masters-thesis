...
So far, we have utilized a rather naive approach of using always the same predetermined starting condition -- we shall refer to this as a *cold* start^[For reference on the terminology, see [cold start Wikipedia entry](https://en.wikipedia.org/wiki/Cold_start_(recommender_systems)).]. Nevertheless, we have no guarantee such choice was optimal for any grid point, much less for the entire grid. 
...
$$
\starter_{\indicesOf{\meshGrid}}(\vi i) = \argmin_{\vi j \in \indicesOf{\meshGrid}} \loss(\vi i, \vi j, \beta_{\vi i}),
